...
def load_dataset_from_feature_store(
    ...
    project = hopsworks.login(api_key_value=SETTINGS["FS_API_KEY"], project=SETTINGS["FS_PROJECT_NAME"])
    ...
    with init_wandb_run(name="load_training_data", job_type="load_feature_view", group="dataset") as run:
        ...
        raw_data_at = wandb.Artifact(name="energy_consumption_denmark_feature_view",type="feature_view",metadata=fv_metadata,)
    ...
    with init_wandb_run(name="train_test_split", job_type="prepare_dataset", group="dataset") as run:
        ...
        for split in ["train", "test"]:
            ...
            artifact = wandb.Artifact(name=f"split_{split}", type="split", metadata=split_metadata,)
